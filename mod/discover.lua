...
local function get_bb(ents)
	...
	for _, b in ipairs(ents) do
		...
		if min.x == nil or bb.left_top.x < min.x then
			min.x = bb.left_top.x
		end
		if min.y == nil or bb.left_top.y < min.y then
			min.y = bb.left_top.y
		end
		if max.x == nil or bb.right_bottom.x > max.x then
			max.x = bb.right_bottom.x
		end
		if max.y == nil or bb.right_bottom.y > max.y then
			max.y = bb.right_bottom.y
		end
	end
...
local function find_belts(self, belt)
	local function fetch_tl(ent, index)
		...
		if self.input then
			tl = belt.get_transport_line(index).input_lines
		end
...
local function find_inserter_neighbor(self, ent)
	...
	if self.input then
		pos = ent.pickup_position
	end

	local found_ents = find_entities_filtered({ position = pos })
	for _, e in ipairs(found_ents) do
		if is_relevant(e) then
			return e
		end
	end
